...
def request(flow):
    ...
    slack_msg = {
        ...
        "blocks":[
            {"type":"section","text":{"type":"mrkdwn","text":"["+proto.upper()+"] request from `"+fromip+"` to `"+host+flow.request.path+"`"}},
            {"type":"section","text":{"type":"mrkdwn","text":"```"+req[:2500]+("[...]" if len(req) > 2500 else "") + "```"}}
        ]
    }

    discord_msg = {
        "content": "["+proto.upper()+"] request from `"+fromip+"` to `"+host+flow.request.path+"`",
        "embeds": [{
